...
exports.findAllUser =  () => {
  let _sql = `select user.*,unit.name as unit_name,room.name as room_name,job.name as job_name,job.area as job_area from user,unit,room,job where user.unit_id = unit.id and user.room_id = room.id and user.job_id = job.id`
  return query( _sql)
}

exports.findUserById = (value) => {
  let _sql = `select user.*,unit.name as unit_name,room.name as room_name,job.name as job_name,job.area as job_area from user,unit,room,job where user.unit_id = unit.id and user.room_id = room.id and user.job_id = job.id and user.id = ?;`
  return query( _sql, value)
}