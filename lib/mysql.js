...
exports.findUsersByRoomId =  (value) => {
  let _sql = `select * from user where room_id = ?;`
  return query( _sql, value)
}
...
exports.insertUser = ( value ) => {
  let _sql = "insert into user set name=?,sex=?,phone=?,unit_id=?,room_id=?,job_id=?;"
  return query( _sql, value )
}
...
exports.updateUser = ( value ) => {
  let _sql = "update user set name=?,sex=?,phone=?,unit_id=?,room_id=?,job_id=? where id=?;"
  return query( _sql, value )
}

exports.findAllUser =  () => {
  let _sql = `select user.id,user.name,user.sex,user.phone,unit.name as unit_name,room.name as room_name,job.name as job_name,job.area as job_area from user,unit,room,job where user.unit_id = unit.id and user.room_id = room.id and user.job_id = job.id`
  return query( _sql)
}

exports.findUserById = (value) => {
  let _sql = `select user.id,user.name,user.sex,user.phone,unit.name as unit_name,room.name as room_name,job.name as job_name,job.area as job_area from user,unit,room,job where user.unit_id = unit.id and user.room_id = room.id and user.job_id = job.id and user.id = ?;`
  return query( _sql, value)
}